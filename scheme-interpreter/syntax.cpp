...
public:
  ...
  cond_expr (sc_obj obj_) {
    ...
    while (is_pair(obj_)) {
      let as_cons = get<cons*>(obj_);
      if (!holds_alternative<cons*>(as_cons->car)) {
        throw runtime_error("cond_expr::cond_expr: type error\n");
      }
      let new_clause = clause(get<cons*>(as_cons->car));
...
make_definition(cons *obj_) {
  ...
  if (holds_alternative<symbol>(cadr)) {  
    ...
    let parameters = obj_->at("cdadr");
    let body = obj_->at("cddr");
...
classify(sc_obj obj) {
  if (is_pair(obj)) {
    ...
    if (holds_alternative<symbol>(p->car)) {
      let tag = get<symbol>(p->car);
      let found = special_forms.find(tag.name);
      if (found != special_forms.end()) {
        let func = found->second;
        return func(p);
      }
